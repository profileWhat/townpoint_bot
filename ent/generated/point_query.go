...
// QueryTown chains the current query on the "town" edge.
func (pq *PointQuery) QueryTown() *TownQuery {
	query := (&TownClient{config: pq.config}).Query()
...
func (pq *PointQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (pq *PointQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (pq *PointQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (pq *PointQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (pq *PointQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (pq *PointQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := pq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
// WithTown tells the query-builder to eager-load the nodes that are connected to
// the "town" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PointQuery) WithTown(opts ...func(*TownQuery)) *PointQuery {
	query := (&TownClient{config: pq.config}).Query()
...
func (pq *PointQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Point, error) {
	...
	if query := pq.withTown; query != nil {
		if err := pq.loadTown(ctx, query, nodes, nil,
			func(n *Point, e *Town) { n.Edges.Town = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PointQuery) loadTown(ctx context.Context, query *TownQuery, nodes []*Point, init func(*Point), assign func(*Point, *Town)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Point)
	for i := range nodes {
		fk := nodes[i].TownID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "town_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PointQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(point.Table, point.Columns, sqlgraph.NewFieldSpec(point.FieldID, field.TypeUUID))
	_spec.From = pq.sql
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withTown != nil {
			_spec.Node.AddColumnOnce(point.FieldTownID)
		}
	}
